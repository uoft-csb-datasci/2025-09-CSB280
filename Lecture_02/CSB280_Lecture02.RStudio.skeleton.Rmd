...
When we refer to the kind of value a variable might point to, we use the name "data type" to describe this value. These basic data types can also be described as atomic values. Data types are used to classify the basic spectrum of values that are used in R. Here's a table describing *some* of the common data types we'll encounter.
...
Variables, as we've seen, can be assigned to one single value from any of the above atomic values. While it is the smallest possible "unit" of data within R, as we'll soon see, they are actually not stored as their own special data structure.
